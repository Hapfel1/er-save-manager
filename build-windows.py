...
# Get version from pyproject.toml instead of importing er_save_manager
...
build_exe_options = {
    ...
    "build_exe": f"dist/windows-{VERSION}/Elden_Ring_Save_Manager_{VERSION}",
    # Optimize .pyc files (2 strips docstrings)
    "optimize": 2,
}

# Base for the executable (console app for CLI tool)
base = None
if sys.platform == "win32":
    # Keep console visible for CLI tool
    base = None  # or "Console" explicitly
...
executables = [
    Executable(
        ...
        target_name="Elden_Ring_Save_Manager",
        # Path to the icon file (if you have one)
        icon=None,
    )
]
