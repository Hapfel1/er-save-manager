...
class DS3CharacterManagementTab:
    ...
    def _slot_names(self, save=None) -> list[str]:
        ...
        if not save:
            return [f"Slot {i + 1} - Empty" for i in range(DS3Save.CHARACTER_SLOTS)]
        names = []
    ...
    def _confirm_empty_target(self, to_slot: int) -> bool:
        ...
        Dark Souls III will not display a character written directly into a
        slot that has never been created in-game. The save file looks correct
        and the manager will show it, but the in-game load screen stays blank
        for that slot. This is a game limitation, not a save-file bug -
        confirmed against real saves and documented by the modding community
        since at least 2016. The only known workaround is to create a
        throwaway character in that slot in-game first, then overwrite it
        here.
        """
    ...
    def _select_target_save_file(self) -> str | None:
        ...
        profile = PROFILES_BY_KEY["dark_souls_3"]
    ...
    def _pick_target_slot(self, target_save: DS3Save, target_path: str) -> int | None:
