...
class NRCharacterManagementTab:
    ...
    def _confirm_empty_target(self, to_slot: int) -> bool:
        ...
        stays blank for that slot. This is a game limitation, not a
        save-file bug - the same behavior has long been documented in the
        modding community for Dark Souls Remastered and Dark Souls III, and
        is confirmed to also apply here. The only known workaround is to
