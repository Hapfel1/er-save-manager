...
def _weapon_level_from_item(item: DSRItem) -> int:
    """
    Convert item upgrade level to weapon level (WL) used for matchmaking.

    DS1 matchmaking uses a 0-15 WL scale:
      MaxUpgrade=15 (standard path): WL = upgrade level (0-15)
      MaxUpgrade=10 (unique path):   WL = 5 + upgrade level (5-15)
      MaxUpgrade=5  (boss path):     WL = 10 + upgrade level (10-15)
    Infused weapons cap at MaxUpgrade 5 or 10 depending on infusion type.
    """
    ul = item.upgrade_level
    # Pyromancy Flame (Ascended) is always WL 15
    if item.base_item_id == 0x145520:
        return 15
    # Guess from upgrade level alone since we don't carry db here;
    # best effort without item database
    return min(ul, 15)
