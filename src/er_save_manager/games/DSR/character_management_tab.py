...
class DSRCharacterManagementTab:
    ...
    def _setup_export_panel(self) -> None:
        ctk.CTkLabel(
            self._ops_scrollable,
            text=t("Save character to a standalone .dsrc file for backup or sharing"),
            font=("Segoe UI", 11),
            text_color=("gray40", "gray70"),
        ).pack(anchor=tk.W, pady=10)
        controls = ctk.CTkFrame(self._ops_scrollable, fg_color="transparent")
    ...
    def _setup_import_panel(self) -> None:
        ctk.CTkLabel(
            self._ops_scrollable,
            text=t("Load a character from a .dsrc file into a slot"),
            font=("Segoe UI", 11),
            text_color=("gray40", "gray70"),
        ).pack(anchor=tk.W, pady=10)
        ctk.CTkLabel(
    ...
    def _check_game_not_running(self) -> bool:
        if self.is_game_running and self.is_game_running():
            CTkMessageBox.showerror(
                t("Game is running"),
                t("Please close Dark Souls: Remastered before modifying save files."),
                parent=self.parent,
            )
            return False
        return True
