...
class SaveManagerGUI:
    ...
    def setup_quick_fix_tab(self):
        ...
        action_frame = ttk.LabelFrame(
            self.tab_quick_fix, text="Actions", padding="10"
        )
    ...
    def setup_stats_editor(self, parent):
        ...
        ttk.Label(other_frame, text="Level:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        self.level_var = tk.IntVar(value=0)
        ttk.Entry(other_frame, textvariable=self.level_var, width=10).grid(
            row=0, column=1, padx=5, pady=5
        )

        ttk.Label(other_frame, text="Runes:").grid(row=0, column=2, sticky=tk.W, padx=5, pady=5)
        ...
        ttk.Button(
            button_frame, text="Revert", command=self.load_character_for_edit, width=15, style="Accent.TButton",
        ).pack(side=tk.LEFT, padx=5)
    ...
    def setup_world_tab(self):
        ...
        # Teleport
        teleport_frame = ttk.LabelFrame(self.tab_world, text="Teleportation", padding=10)
        teleport_frame.pack(fill=tk.X, pady=10)

        ttk.Label(teleport_frame, text="Teleport to safe location:").pack(anchor=tk.W, pady=5)
    ...
    def setup_advanced_tab(self):
        ...
        # Save info
        info_frame = ttk.LabelFrame(self.tab_advanced, text="Save Information", padding=10)
        ...
        try:
            ...
            for slot_idx in active_slots:
                try:
                    ...
                    self.char_listbox.insert(tk.END, f"Slot {slot_idx + 1:2d} | Error loading data")
    ...
    def show_character_details(self, event=None):
        ...
        if is_dlc_location:
            info.append(f"  WARNING: Currently in DLC area!")
        info.append("")
        ...
        if has_dlc_flag or has_invalid_dlc:
            ...
            if has_invalid_dlc:
                info.append(f"  WARNING: Invalid data in unused DLC slots")
            info.append("")
        ...
        if issues_detected:
            ttk.Button(
                button_frame,
                text="Fix All Issues",
                command=lambda: self.fix_character_from_dialog(dialog, slot_idx, len(issues_detected)),
                width=20,
                style="Accent.TButton",
            ).pack(side=tk.LEFT, padx=5)
    ...
    def teleport_from_dialog(self, dialog, slot_idx):
        ...
        x = (teleport_dialog.winfo_screenwidth() // 2) - (teleport_dialog.winfo_width() // 2)
        y = (teleport_dialog.winfo_screenheight() // 2) - (teleport_dialog.winfo_height() // 2)
        ...
        location_frame = ttk.LabelFrame(teleport_dialog, text="Select Destination", padding=10)
    ...
    def quick_fix_all(self):
        ...
        try:
            ...
            # Apply fix
            was_fixed, fixes = self.save_file.fix_character_corruption(self.selected_slot)
        ...
        try:
            ...
            if hasattr(slot, "player_game_data") and slot.player_game_data:
                ...
                try:
                    ...
                    messagebox.showinfo("Loaded", f"Character stats loaded for Slot {slot_idx + 1}")
                except Exception as e:
                    messagebox.showerror("Error", f"Failed to load some stats:\n{str(e)}")
            else:
                messagebox.showerror("Error", "Could not load character data - player_game_data missing")
    ...
    def apply_stat_changes(self):
        ...
        try:
            ...
            if hasattr(slot, "player_game_data") and slot.player_game_data:
                ...
                if hasattr(slot, 'player_game_data_offset') and slot.player_game_data_offset >= 0:
                    ...
                    if len(char_data) != 432:  # PlayerGameData is exactly 432 bytes
                        raise RuntimeError(f"PlayerGameData serialization error: expected 432 bytes, got {len(char_data)}")
                    ...
                    abs_offset = slot_start + CHECKSUM_SIZE + slot.player_game_data_offset
                    
                    # Write to raw data
                    self.save_file._raw_data[abs_offset : abs_offset + len(char_data)] = char_data
    ...
    def import_preset_from_json(self):
        ...
        try:
            ...
            preset_combo = ttk.Combobox(
                frame, textvariable=preset_var, values=preset_options, state="readonly", width=40
            )
            ...
            ttk.Button(
                button_frame, text="Cancel", command=dialog.destroy, width=15, style="Accent.TButton",
            ).pack(side=tk.LEFT, padx=5)
    ...
    def apply_preset_to_character(self):
        ...
        try:
            ...
            if preset.is_empty():
                messagebox.showwarning("Empty Preset", f"Preset {preset_idx + 1} is empty")
                return
            ...
            ttk.Label(info_frame, text=f"Face Model: {preset.face_model}").pack(anchor=tk.W)
            ttk.Label(info_frame, text=f"Hair Model: {preset.hair_model}").pack(anchor=tk.W)

            slot_frame = ttk.LabelFrame(dialog, text="Select Character Slot", padding=10)
            ...
            def do_import():
                ...
                try:
                    ...
                    if slot.is_empty():
                        messagebox.showerror("Error", f"Slot {target_slot + 1} is empty")
                        return
                    ...
                    if len(preset_data) != 0x130:
                        raise RuntimeError(f"Preset size mismatch: {len(preset_data)} != 0x130")

                    HEADER_SIZE = 0x300 if self.save_file.magic == b"BND4" else 0x6C
                    SLOT_SIZE = 0x280000
                    CHECKSUM_SIZE = 0x10

                    slot_start = HEADER_SIZE + (target_slot * (SLOT_SIZE + CHECKSUM_SIZE))
                    ...
                    if len(preset_face_data) < 0x12F:
                        raise RuntimeError(f"Preset face data too short: {len(preset_face_data)}")

                    face_data_size = 0x12F
                    self.save_file._raw_data[face_offset : face_offset + face_data_size] = preset_face_data[:face_data_size]
            ...
            def refresh_list():
                ...
                for backup in backups:
                    ...
                    tree.insert(
                        ...
                        values=(timestamp, backup.operation, backup.description, size_mb),
                    )
                ...
                if info:
                    ...
                    if info.character_summary:
                        ...
                        for char in info.character_summary:
                            details.append(f"  Slot {char['slot']}: {char['name']} (Lv.{char['level']})")
