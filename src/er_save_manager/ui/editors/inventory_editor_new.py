...
class InventoryEditor:
    ...
    def add_item(self):
        ...
        try:
            ...
            # Dump first 10 non-zero gaitem_game_data entries to understand structure
            ggd = slot.gaitem_game_data
            log.debug(f"add_item: gaitem_game_data count={ggd.count}")
            shown = 0
            for i, e in enumerate(ggd.entries):
                if e.id != 0 or e.next_item_id != 0:
                    log.debug(
                        f"add_item: ggd[{i}] id=0x{e.id:08X} unk0x4=0x{e.unk0x4:02X} next=0x{e.next_item_id:08X} unk0xc=0x{e.unk0xc:02X}"
                    )
                    shown += 1
                    if shown >= 15:
                        log.debug(f"add_item: ggd ... ({ggd.count} total)")
                        break
